...
async fn fetch_extras(client: &Client, url: &str, api_key: &str) -> serde_json::Value {
    // Get all movies and count missing (monitored but no file)
    let (total, missing) = match client
        .get(format!("{}/api/v3/movie?apikey={}", url, api_key))
        .send().await
    {
        Ok(resp) if resp.status().is_success() => {
            match resp.json::<Vec<Movie>>().await {
                Ok(movies) => {
                    let total = movies.len() as i64;
                    let missing = movies.iter().filter(|m| m.monitored && !m.has_file).count() as i64;
                    (total, missing)
                }
                _ => (0, 0),
            }
...
pub async fn list_movies(client: &Client, url: &str, api_key: &str) -> Result<serde_json::Value, reqwest::Error> {
    let endpoint = format!("{}/api/v3/movie?apikey={}", url, api_key);
    client.get(&endpoint).send().await?.json().await
}

pub async fn search_movies(client: &Client, url: &str, api_key: &str, term: &str) -> Result<serde_json::Value, reqwest::Error> {
    let endpoint = format!("{}/api/v3/movie/lookup?apikey={}&term={}", url, api_key, urlencoding::encode(term));
    client.get(&endpoint).send().await?.json().await
}

pub async fn add_movie(client: &Client, url: &str, api_key: &str, body: serde_json::Value) -> Result<serde_json::Value, reqwest::Error> {
    let endpoint = format!("{}/api/v3/movie?apikey={}", url, api_key);
    client.post(&endpoint).json(&body).send().await?.json().await
}

pub async fn delete_movie(client: &Client, url: &str, api_key: &str, id: i64, delete_files: bool) -> Result<(), reqwest::Error> {
    let endpoint = format!("{}/api/v3/movie/{}?apikey={}&deleteFiles={}", url, id, api_key, delete_files);
...
pub async fn get_calendar(client: &Client, url: &str, api_key: &str, start: &str, end: &str) -> Result<serde_json::Value, reqwest::Error> {
    let endpoint = format!("{}/api/v3/calendar?apikey={}&start={}&end={}", url, api_key, start, end);
    client.get(&endpoint).send().await?.json().await
}
